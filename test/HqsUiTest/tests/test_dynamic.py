...
class HqsTestCases(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.title = cls.dynamic.get('title')
        cls.content = cls.dynamic.get('content')

        print(cls.title, cls.content)
    ...
    def test_video_is_ok(self):
        ...
        self.lib.send_keys(self.video_title, self.title)
        self.lib.send_keys(self.video_content, self.content)
        self.lib.send_keys(self.video_xpath,self.video_file)
        self.lib.click(self.video_send)
        time.sleep(10)  # 上传视频时间，上传小于3min的视频

        self.assertEqual('标题', self.lib.parse(self.assert_dynamic)[0])
    ...
    def test_music_is_ok(self):
        ...
        time.sleep(1)
        self.lib.send_keys(self.music_title, self.title)
        self.lib.send_keys(self.music_content, self.content)
        self.lib.click(self.music_select)
        print(self.music_file)
        os.system("upload.exe %s" %self.music_file)
        time.sleep(5)
    ...
    def test_photo_is_ok(self):
        ...
        time.sleep(1)
        self.lib.send_keys(self.photo_title, self.title)
        self.lib.send_keys(self.photo_content, self.content)
